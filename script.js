(function(){
  ...
  const el={
    ...
    cardPrompt:$('cardPrompt'), cardTitle:$('cardTitle'), cardDesc:$('cardDesc'), descRow:$('cardDescRow'), descLeft:$('cardDescLeft'), descRight:$('cardDescRight'),
...
function handleActedIPSelection(ip){
  ...
  function setCardUI(prompt, desc, leftText, rightText, ip){
    ...
    el.cardDesc.textContent=desc;
     el.descLeft && (el.descLeft.textContent = '左：'+leftText);
    el.descRight && (el.descRight.textContent = '右：'+rightText);
el.badgeLeft.textContent='左：'+leftText;
    ...
    try{
      if(ip){
        const wL = getWeightsFor(ip, leftText);
        const wR = getWeightsFor(ip, rightText);
        const pfL = '粉x'+(wL.fans.toFixed(2))+' 金x'+(wL.coins.toFixed(2));
        const pfR = '粉x'+(wR.fans.toFixed(2))+' 金x'+(wR.coins.toFixed(2));
        if(el.descLeft)  el.descLeft.innerHTML  = `左：${leftText}<br><span class="meter ${classify(wL.fans*wL.coins)}">粉x${wL.fans.toFixed(2)} 金x${wL.coins.toFixed(2)}</span>`;
        if(el.descRight) el.descRight.innerHTML = `右：${rightText}<br><span class="meter ${classify(wR.fans*wR.coins)}">粉x${wR.fans.toFixed(2)} 金x${wR.coins.toFixed(2)}</span>`;
      }
    }catch(e){}
/*__RESIZE_BADGE_AREA__*/
    try{
      // Let browser layout update first
      requestAnimationFrame(()=>{
        const hL = el.badgeLeft ? el.badgeLeft.offsetHeight : 0;
        const hR = el.badgeRight ? el.badgeRight.offsetHeight : 0;
        const h = Math.max(hL, hR) + 12; // add breathing space
        if(el.choiceCard && h>0){ el.choiceCard.style.setProperty('--choice-badge-area', h+'px'); }
      });
    }catch(e){}
}
...
// --- Topic tag & weight helpers ---
function getTagForTitle(title){
  try{
    const t = (typeof window.__topicByTitle==='function') ? window.__topicByTitle(title) : null;
    return t && t.tag ? t.tag : null;
  }catch(e){ return null; }
}
function applyTagWeights(result, title, ip){
  try{
    const tag = getTagForTitle(title) || '其他';
    const W = getWeightsFor(ip, title);
    if(W){
      if(typeof result.fans==='number')  result.fans  = Math.max(0, Math.round(result.fans  * (W.fans||1)));
      if(typeof result.coins==='number') result.coins = Math.max(0, Math.round(result.coins * (W.coins||1)));
      result._tag = W.tag; // for debugging/toast
    }
  }catch(e){}
  return result;
}
...
// Map label (either full title or generic tag) to usable tag
function getTagForLabelOrTitle(label){
  if(!label) return null;
  // If it's an exact topic title, try topic record first
  const byTitle = (typeof window.__topicByTitle==='function') ? window.__topicByTitle(label) : null;
  if(byTitle){
    const override = (typeof window.__inferTagByTitle==='function') ? window.__inferTagByTitle(label) : null;
    return override || byTitle.tag || null;
  }
  // Otherwise, treat label as tag if it matches common tags
  const known = ['测评','挑战','教程','盘点','采访','探店/攻略','改造','vlog','实验','案例','其他'];
  if(known.includes(label)) return label;
  // Heuristic override
  const heur = (typeof window.__inferTagByTitle==='function') ? window.__inferTagByTitle(label) : null;
  return heur || null;
}

// Get weights by (industry×tag) with fallbacks to global tag/default
function getWeightsFor(ip, label){
  const tag = getTagForLabelOrTitle(label) || '其他';
  const mapInd = (typeof window.__mapIndustry==='function') ? window.__mapIndustry(ip?.industry) : (ip?.industry || null);
  const ind = mapInd || 'unknown';
  const TW = (window.TOPIC_WEIGHTS||{});
  const GW = (window.TOPIC_TAG_WEIGHTS||{});
  let w = null;
  if(TW[ind]){
    const titles = TW[ind]['_titles']||{};
    if(label && titles[label]) w = titles[label];
    else if(TW[ind][tag]) w = TW[ind][tag];
  }
  if(!w) w = GW[tag] || (TW['_tags']?TW['_tags'][tag]:null) || (TW['_default']||{fans:1,coins:1});
  return { fans: (w.fans||1), coins: (w.coins||1), tag };
}

// Pretty UI meter
function classify(val){
  if(val>=1.30) return 'meter-hot';
  if(val>=1.15) return 'meter-good';
  if(val<=0.90) return 'meter-bad';
